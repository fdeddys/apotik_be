...
func GetOrderDetailPage(param dto.FilterOrderDetail, offset, limit int) ([]dbmodels.SalesOrderDetail, int, error) {
	...
	go AsyncQuerysOrderDetails(db, offset, limit, &orderDetails, order.ID, errQuery)
	go AsyncQueryCountsOrderDetails(db, &total, order.ID, offset, limit, errCount)
...
func AsyncQuerysOrderDetails(db *gorm.DB, offset int, limit int, orderDetails *[]dbmodels.SalesOrderDetail, orderID int64, resChan chan error) {
	...
	err = db.Offset(offset).Limit(limit).Preload("Product").Preload("Lookup", "lookup_group = ?", "UOM").Find(&orderDetails, " order_id = ? ", orderID).Error
	if err != nil {
